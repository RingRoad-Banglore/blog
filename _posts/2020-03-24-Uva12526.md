---
title: Uva 12526  Cellphone Typing
---
...
letter by pressing the corresponding key. In this way, a user needs P keystrokes to type a word of length
P .
...
1. The system never guesses the first letter of a word, so the first letter always has to be input
...
needs just one keystroke, because after pressing the initial g the system will automatically fill in the
...
number of words in the dictionary (1  N  105). Each of the next N lines contains a non-empty
string of at most 80 lowercase letters from the English alphabet, representing a word in the dictionary.
Within each test case all words are different, and the sum of the lengths of all words is at most 106.
...
Universidad de Valladolid OJ: 12526  Cellphone Typing 2/2
