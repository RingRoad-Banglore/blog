...
20 road segments intersect at a given intersection. The location of houses and firestations alike are
...
There may be more than one firestation per intersection.
```
...
The first line of input contains two positive integers: f , the number of existing fire stations (f  100)
and i, the number of intersections (i  500). The intersections are numbered from 1 to i consecutively.
f lines follow; each contains the intersection number at which an existing fire station is found. A
...
road segments are two-way (at least as far as fire engines are concerned), and there will exist a route
