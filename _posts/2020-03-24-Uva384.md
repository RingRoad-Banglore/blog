...
2. The first character is followed by a string of one or more Fs.

3. The string of one or more Fs is followed by either a Slump or a G. The Slump or G that
follows the Fs ends the Slump. For example DFFEFFFG is a Slump since it has a D for its first
character, followed by a string of two Fs, and ended by the Slump EFFFG.
...
Universidad de Valladolid OJ: 384  Slurpys 2/2
