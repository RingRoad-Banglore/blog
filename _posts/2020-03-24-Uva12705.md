---
title: Uva 12705  Breaking Board
---
...
Hector Salamanca, the cartel don aged before his years and is always confined to his wheelchair and
...
choosing space of a sentence is 0. For Example, cost of making sentence CALL DEA is (1+3)
+ (1+1) + (3+4) + (3+4) + (1+4) + (2+1) + (1+1) = 30.

In our problem Hector has a sentence to complete but the board is not fixed. We can break the
...
This can be an optimal formation of board. Then the cost will be (1+2) + (1+1) + (2+1) + (2+1)
+ (1+3) + (3+1) + (1+1) = 21.
```
...
Universidad de Valladolid OJ: 12705  Breaking Board 2/2
