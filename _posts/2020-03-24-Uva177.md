...
opened half-way up, so every crease forms a 90 degree angle,
then (viewed end-on) it forms a dragon curve. For example, if
four successive folds are made, then the following curve is seen
(note that it does not cross itself, but two corners touch):

Write a program to draw the curve which appears after N
folds. The exact specification of the curve is as follows:
...
(marked s). Horizontal pieces are to be displayed with the underscore character  , and vertical pieces
with the | character.
```
...
folds is terminated by a line containing a single .
...
Universidad de Valladolid OJ: 177  Paper Folding 2/2
