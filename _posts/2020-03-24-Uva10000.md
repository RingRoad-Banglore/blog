...
Csar has this kind of problem. When he has to go from one point to another he realizes that he
has to visit many people, and thus always chooses the longest path. One of Csars friends, Felipe, has
understood the nature of the problem. Felipe thinks that with the help of a computer he might be able
to calculate the time that Csar is going to need to arrive to his destination. That way he could spend
his time in something more enjoyable than waiting for Csar.

Your goal is to help Felipe developing a program that computes the length of the longest path that
can be constructed in a given graph from a given starting point (Csars residence). You can assume
that there is at least one path from this starting point to any destination. Also, the graph has no cycles
(there is no path from any node to itself), so Csar will reach his destination in a finite time. In the
...
(1 < n  100) that specifies the number of points that Csar might visit (i.e., the number of nodes in
the graph).

A value of n = 0 indicates the end of the input.

After this, a second number s is provided, indicating the starting point in Csars journey (1  s 
n). Then, you are given a list of pairs of places p and q, one pair per line, with the places on each line
separated by white-space. The pair p q indicates that Csar can visit q after p.
...
Universidad de Valladolid OJ: 10000  Longest Paths 2/2
