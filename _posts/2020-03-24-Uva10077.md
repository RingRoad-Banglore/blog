---
title: Uva 10077  The SternBrocot Number System
---
...
n+n
between two adjacent fractions m
...
The construction preserves order, and we couldnt possibly get the same fraction in two different
places.

We can, in fact, regard the Stern-Brocot tree as a number system for representing rational numbers,
because each positive, reduced fraction occurs exactly once. Lets use the letters L and R to stand for
going down to the left or right branch as we proceed from the root of the tree to a particular fraction;
then a string of Ls and Rs uniquely identifies a place in the tree. For example, LRRL means that we go
left from 1
...
representation of 5
7
. Every positive fraction gets represented in this way as a unique string of Ls and
...
Well, actually theres a slight problem: The fraction 1
1

corresponds to the empty string, and we
need a notation for that. Lets agree to call it I, because that looks something like 1 and it stands for
identity.
...
m and n where m and n are relatively prime. The input terminates with a test case containing two 1s
