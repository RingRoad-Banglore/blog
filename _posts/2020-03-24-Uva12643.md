...
the rounds draw as it establishes the matches that will be played among the players. In particular, the
first round matches are numbered 1, 2, . . . , 2N1: match k will have player 2  k  1 vs. player 2  k, for
1  k  2N1.
...
of the first round match k would be reassigned the number k, then the second rounds draw may be
defined exactly as already explained for the first round. This assignment process could be repeated
over and over again until there is exactly one player remaining, who happens to be the tournaments
champion:
...
and i, j represent two seeding numbers at the first round (1  i  2N , 1  j  2N , i = j).
...
Universidad de Valladolid OJ: 12643  Tennis Rounds 2/2
