...
The first line of each dataset contains two integers: a positive integer n (0 < n  50) giving the
length of the strings; and a positive integer m (0 < m  100) giving the number of strings. These are
...
```
For each dataset, output the list of input strings, arranged from most sorted to least sorted. If two
or more strings are equally sorted, list them in the same order they are in the input file.
