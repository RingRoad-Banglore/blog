...
Your task is to calculate the number of subsets of nodes of the graph with the following properties:
 ...
 it shouldnt be possible to add further nodes to the subset without violating the first condition

For a graph with 5 nodes the number of subsets which fulfill the above conditions is 4. The subsets
are {1,3,5},{2,4},{2,5},{1,4}.
```
