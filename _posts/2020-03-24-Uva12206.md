...
```text
The input contains several test cases. Each test case consists of a line with an integer m (m  1), the
minimum number of repetitions, followed by a line containing a string s of length between m and 40 000,
inclusive. All characters in s are lowercase characters from a to z. The last test case is denoted by
