...
compute the remainder r(x) when
a(x) is divided by xk + 1.
```
...
cases. The first line of each case specifies the two integers n and k (0  n, k  10000). The next n+ 1
integers give the coefficients of a(x), starting from a0 and ending with an. The input is terminated if
n = k = 1.
