...
of the find string within the text by the replace by string, then try to perform the same replacement
...
for a find string, you always start searching at the beginning of the text, (2) once you have finished
using a rule (because the find string no longer occurs) you never use that rule again, and (3) case is
significant.

For example, suppose we start with the line
banana boat

and apply these rules. The sequence of transformations is shown below, where occurrences of a find
...
1 and 10. Each rule is specified by a pair of lines, where the first line is the find string and the second
...
Note: Both find and replace by strings will be at most 80 characters long. find strings will contain
...
Universidad de Valladolid OJ: 10115  Automatic Editing 2/2
